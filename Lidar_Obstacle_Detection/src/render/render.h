...
struct Car
{
	...
	{
		return (center-range <= point) and (center+range >= point);
	}

	bool checkCollision(Vect3 point)
	{
		return (inbetween(point.x,position.x,dimensions.x/2)
		        and inbetween(point.y,position.y,dimensions.y/2)
		        and inbetween(point.z,position.z+dimensions.z/3,dimensions.z/3))
		        or
			   (inbetween(point.x,position.x,dimensions.x/4)
			   and inbetween(point.y,position.y,dimensions.y/2)
			   and inbetween(point.z,position.z+dimensions.z*5/6,dimensions.z/6));
